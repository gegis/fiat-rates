...
class FiatRates {
  constructor(options) {
    ...
    if (cacheConfig.persist.clearOnStartup) {
      this.clearCache();
    }
  ...
  getLatestDate(formatDate = false) {
    const { format, timezone, subtractMinutes } = this.config.date;
    const latest = moment().tz(timezone);
    if (subtractMinutes) {
      latest.subtract(subtractMinutes, 'minutes');
    }
    if (formatDate) {
